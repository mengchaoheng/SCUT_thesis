...
\begin{document}
	...
	% \include{chapter/chapter05} % 新增公式字体设置，需要引入字体包unicode-math然后通过setmainfont设置不同字体（unicode-math包默认Latin Modern Math字体，而latex默认Computer Modern Math字体）， Computer Modern Math也是模板一直以来使用的字体。这里提供给需要更多字体的同学探索。模版的修改在scutthesis.cls文件82-85行，自行探索各种常用数学符号的区别，编译math_font文件夹的各种测试文件。
	% 自行根据需要添加章节。
